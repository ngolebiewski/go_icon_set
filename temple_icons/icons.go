...
func Itchio(cfg ...IconConfig) templ.Component {
	return templ.ComponentFunc(func(ctx context.Context, w io.Writer) error {
		style := buildStyleAttr(cfg)
		_, err := io.WriteString(w, fmt.Sprintf("<svg xmlns=\"http://www.w3.org/2000/svg\" viewBox=\"0 0 16 16\" shape-rendering=\"crispEdges\" fill=\"currentColor\" %s><path d=\"M12,9h1v1h-1z\" /></svg>", style))
		return err
	})
}
